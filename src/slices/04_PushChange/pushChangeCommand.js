 ...
 */
export function pushChangeCommand(changeId, currentChangeStatus) {
  ...
  if (currentChangeStatus !== "Committed") {
    ...
    console.warn(`Push command failed: Change with ID ${changeId} has status "${currentChangeStatus}". Expected "Committed".`);
  ...
  const suggestedEvent = {
    ...
    timestamp: Date.now(),
    payload: {
      status: "Pushed" // The new status after this push
    }
  };
